...
const VendasTab = () => (
  ...
  const MasterPanel = () => {
    ...
    const storeNameFromRow = (row) => {
      const data = safeData(row);
      const profile = data.erpmini_company_profile || {};
      return profile.nomeFantasia || data.erpmini_storename || profile.razaoSocial || "Sem nome";
    };

    const licenseForRow = (row) => {
      const email = ownerEmailFromRow(row);
      if (!email) return null;
      return masterLicenses.find(l => String(l.email || "").trim().toLowerCase() === email) || null;
    };
    ...
    const rowSummary = (row) => {
      ...
      const store = storeNameFromRow(row);
      const ownerEmail = ownerEmailFromRow(row);
      const license = licenseForRow(row);
    ...
    const saveMasterLicense = async (email, patch) => {
      const cleanEmail = String(email || "").trim().toLowerCase();
      if (!cleanEmail || !cleanEmail.includes("@")) {
        keepMasterMsg("Esta loja ainda não tem e-mail vinculado. Peça para o usuário abrir o ERP atualizado com internet.");
        return;
      }

      const current = masterLicenses.find(l => String(l.email || "").trim().toLowerCase() === cleanEmail) || {};
      const payload = {
        email: cleanEmail,
        status: patch.status ?? current.status ?? "ativo",
        plan: normalizePlan(patch.plan ?? current.plan ?? "starter"),
        expires_at: patch.expires_at !== undefined ? patch.expires_at : (current.expires_at || null),
        notes: patch.notes ?? current.notes ?? null,
        updated_at: new Date().toISOString()
      };

      if (payload.plan === "starter") payload.expires_at = null;
      ...
      if (error) {
        keepMasterMsg("Erro ao salvar licença: " + error.message);
        return;
      }

      keepMasterMsg("Licença atualizada.");
      await loadMaster();
    };

    const changePlan = async (summary, plan) => {
      const p = normalizePlan(plan);
      let expiresAt = summary.license?.expires_at || null;
      ...
      if (p === "premium" && !expiresAt) {
        const d = new Date();
        d.setFullYear(d.getFullYear() + 1);
        expiresAt = d.toISOString().slice(0,10);
      }

      await saveMasterLicense(summary.ownerEmail, {
        plan: p,
        status: "ativo",
        expires_at: p === "starter" ? null : expiresAt,
        notes: p === "starter" ? "Plano Starter gratuito." : `Plano ${p} atualizado pelo Painel Master.`
      });
    };

    const changeStatus = async (summary, status) => {
      await saveMasterLicense(summary.ownerEmail, {
        status,
        plan: summary.license?.plan || "starter",
        expires_at: summary.license?.expires_at || null
      });
    };

    const changeExpires = async (summary, value) => {
      await saveMasterLicense(summary.ownerEmail, {
        status: summary.license?.status || "ativo",
        plan: summary.license?.plan || "pro",
        expires_at: value
      });
    };

    const deleteLicense = async (summary) => {
      if (!summary.ownerEmail) {
        keepMasterMsg("Esta loja ainda não tem e-mail vinculado para excluir licença.");
        return;
      }

      const ok = window.confirm(`Excluir acesso/licença do e-mail ${summary.ownerEmail}?`);
      if (!ok) return;
      ...
      const { error } = await supabase
        .from("erpmini_licenses")
        .delete()
        .eq("email", summary.ownerEmail);
      setMasterLoading(false);

      if (error) {
        keepMasterMsg("Erro ao excluir licença: " + error.message);
        return;
      }

      keepMasterMsg("Licença excluída.");
      await loadMaster();
    };

    const deleteStore = async (row, summary) => {
      const ok = window.confirm(`Excluir loja "${summary.store}" da nuvem?\n\nIsso apaga o backup sincronizado desta loja, incluindo produtos, clientes, vendas e contas.\n\nEsta ação não pode ser desfeita.`);
      if (!ok) return;
      ...
      if (error) {
        keepMasterMsg("Erro ao excluir loja: " + error.message);
        return;
      }

      keepMasterMsg("Loja excluída da nuvem.");
      await loadMaster();
    };
    ...
    const filteredRows = masterRows.filter(row => {
      ...
      if (!q) return true;
      return s.store.toLowerCase().includes(q) || s.ownerEmail.toLowerCase().includes(q) || String(row.user_id || "").toLowerCase().includes(q);
    });
    ...
    return (
      <div style={card}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:"10px", marginBottom:"12px" }}>
          <div>
            <div style={{ fontWeight:"900", fontSize:"18px", color:"#0f172a" }}>Painel Master 2.0</div>
            <div style={{ fontSize:"12px", color:"#64748b", fontWeight:"700" }}>Administração de lojas, e-mails, planos e bloqueios.</div>
          </div>
        ...
        <input style={{ ...inp, marginBottom:"12px" }} value={masterSearch} onChange={e=>setMasterSearch(e.target.value)} placeholder="Buscar por loja, e-mail ou ID..." />

        {filteredRows.length === 0 ? (
          <div style={{ color:"#94a3b8", fontWeight:"800", textAlign:"center", padding:"18px 0" }}>
            Nenhuma loja encontrada. Clique em Atualizar para carregar.
          </div>
        ) : (
          <div style={{ display:"grid", gap:"10px" }}>
            {filteredRows.map(row=>{
              const s = rowSummary(row);
              const status = String(s.license?.status || "").toLowerCase();
              const expired = s.license?.expires_at && s.license.expires_at < new Date().toISOString().slice(0,10);
              const active = status === "ativo" && !expired;
              const statusText = active ? "Ativo" : expired ? "Vencido" : s.license ? "Bloqueado" : "Sem plano";
              const statusBg = active ? "#dcfce7" : "#fee2e2";
              const statusColor = active ? "#166534" : "#991b1b";

              return (
                <div key={row.user_id} style={{ border:"1px solid #e2e8f0", borderRadius:"16px", padding:"12px", background:"#fff" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"10px" }}>
                    <div style={{ minWidth:0 }}>
                    ...
                    <span style={{ borderRadius:"999px", padding:"5px 9px", fontSize:"11px", fontWeight:"900", color:statusColor, background:statusBg, whiteSpace:"nowrap" }}>
                      {statusText}
                    </span>
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)", gap:"7px", marginTop:"10px" }}>
                    <div style={{ background:"#f8fafc", borderRadius:"10px", padding:"8px" }}>
                      <div style={{ fontSize:"10px", color:"#64748b", fontWeight:"900" }}>Plano</div>
                      <div style={{ fontWeight:"900" }}>{planLabel(s.license?.plan)}</div>
                    </div>
                    <div style={{ background:"#f8fafc", borderRadius:"10px", padding:"8px" }}>
                      <div style={{ fontSize:"10px", color:"#64748b", fontWeight:"900" }}>Vencimento</div>
                      <div style={{ fontWeight:"900" }}>{s.license?.expires_at || "sem venc."}</div>
                    </div>
                    <div style={{ background:"#f8fafc", borderRadius:"10px", padding:"8px" }}>
                      <div style={{ fontSize:"10px", color:"#64748b", fontWeight:"900" }}>Faturamento</div>
                    ...
                    <div style={{ background:"#f8fafc", borderRadius:"10px", padding:"8px" }}>
                      ...
                      <div style={{ fontWeight:"900", color:"#f97316" }}>{fmtCur(s.credOpen)}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:"8px", marginTop:"10px" }}>
                    ...
                    <input style={inp} type="date" value={s.license?.expires_at || ""} onChange={e=>changeExpires(s, e.target.value)} disabled={!s.ownerEmail || normalizePlan(s.license?.plan || "starter") === "starter" || masterLoading} />
                  </div>

                  <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)", gap:"6px", marginTop:"8px" }}>
                    <button style={{ ...btnSm("#16a34a") }} onClick={()=>changeStatus(s,"ativo")} disabled={!s.ownerEmail || masterLoading}>Liberar</button>
                    <button style={{ ...btnSm("#ef4444") }} onClick={()=>changeStatus(s,"bloqueado")} disabled={!s.ownerEmail || masterLoading}>Bloquear</button>
                    <button style={{ ...btnSm("#f97316") }} onClick={()=>deleteLicense(s)} disabled={!s.ownerEmail || masterLoading}>Excluir acesso</button>
                    <button style={{ ...btnSm("#991b1b") }} onClick={()=>deleteStore(row,s)} disabled={masterLoading}>Excluir loja</button>
                  </div>
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',sans-serif", background:"#f0f4f8", minHeight:"100vh", paddingBottom: isMobile?"80px":"0" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#1a1a2e,#16213e)", color:"#fff", padding:"12px 16px", display:"flex", alignItems:"center", gap:"10px", position:"sticky", top:0, zIndex:50 }}>
        ...
        <span style={{ fontSize:"10px", background:"rgba(255,255,255,0.12)", color:"#cbd5e1", borderRadius:"20px", padding:"2px 6px" }}>v-master-saas2</span>
